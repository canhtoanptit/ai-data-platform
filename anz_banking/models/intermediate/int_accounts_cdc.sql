...
with cdc as (
    ...
    where cdc_timestamp > (
        select coalesce(max(_cdc_timestamp), '1900-01-01'::timestamp_ntz)
        from {{ this }}
    )
    {% endif %}
),

-- Most recent change per account within this batch. QUALIFY lets us filter on
-- a window function without a subquery — idiomatic Snowflake.
latest as (
    select *
    from cdc
